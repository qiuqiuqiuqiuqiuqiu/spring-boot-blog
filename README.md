# 基于spring boot H2的blog系统。

启动main函数，访问http://localhost:8090即可看到界面。

登录可以用：user/password 

数据库管理地址：http://localhost:8090/h2-console

连接字符串：查看application-xxx.properties里面的spring.datasource.url=后面的字符串
帐号sa
密码空
...
<h3 class="panel-body" th:utext="${post.body}">内容</h3>
去除列表页直接展示内容
...
# 以下是源代码的readme信息，英文的，懒得翻译，直接看源码了。
